...
if __package__ in (None, ""):
    ...
    from mainPAP.Model.Pinn.R6P_v3.train import (
        RUN_MODES,
        build_arg_parser,
...
def main() -> None:
    ...
    off_iou = summaries["B_adapter_off"]["test_metrics"]["dip_mask_iou_expert"]
    on_iou = summaries["B_adapter_on"]["test_metrics"]["dip_mask_iou_expert"]
    expert_iou = summaries["D_full"]["test_metrics"]["dip_mask_iou_expert"]
    naive_iou = summaries["D_full"]["test_metrics"]["dip_mask_iou_naive"]
    print("\nKey Comparisons")
    print(f"A. Stage-1 baseline final MSE: {stage1_mse:.6f}")
    print(f"B. Adapter OFF vs ON dip-mask IoU: {off_iou:.4f} -> {on_iou:.4f}")
    print(f"C. Dip expert vs naive Stage-1 minima IoU: {expert_iou:.4f} vs {naive_iou:.4f}")
