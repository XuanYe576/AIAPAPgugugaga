...
def _build_export_freq_axis_hz(freq_hz: torch.Tensor, target_points: int) -> torch.Tensor:
    if target_points <= 0 or freq_hz.numel() == target_points:
        return freq_hz
    start = float(freq_hz[0].item())
    end = float(freq_hz[-1].item())
    return torch.linspace(start, end, target_points, dtype=freq_hz.dtype, device=freq_hz.device)
...
def load_compatible_model_weights(
    ...
    *,
    ...
    print(
        ...
        f"loaded {len(compatible)} tensors, skipped {len(source_state) - len(compatible)} tensors."
    )
    return payload if isinstance(payload, dict) else {}
...
def save_prediction_graphs(
    ...
    plt = load_pyplot()
    export_freq_hz = _build_export_freq_axis_hz(freq_hz, export_curve_points)
    x_values = (export_freq_hz.detach().cpu().numpy() / 1.0e9).tolist()
    model.eval()
    saved = 0
    with torch.no_grad():
        for idx in indices:
            ...
            pred_tensor = model(geom, mat, export_freq_hz).gamma_db.squeeze(0).detach().cpu()
